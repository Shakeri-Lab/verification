...
const DiagnosisGroupingApp = (): React.JSX.Element => {
  ...
  const uploadGroupedData = async (currentGroups: Group[], currentDeleted: Diagnosis[]) => {
    ...
    try {
      const presignedUrlResponse = await fetch(`${API_BASE_URL}/get-presigned-url`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: `${computingId}_grouped_diagnoses.json`, action: 'putObject' }) });
  ...
  useEffect(() => {
    ...
    else { setGroups([]); setSuggestedGroups([]); setDeletedDiagnoses([]); setTotalInitialSuggestions(0); setLoading(false); }
  }, [startConfirmed, computingId]);

  // Effect to update current suggested index
  useEffect(() => {
